...
def training_loop_mereged_labels_of_deep_fashion(opt):
    ...
    scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.7, patience=1,cooldown=1, min_lr=0.00005, verbose=True)
        ...
        for itr in tqdm.tqdm(pbar):
            ...
            if itr % opt.val_freq == 0 and itr!=0:
                ...
                with torch.no_grad():
                    ...
                    for (label,iou_list), (_, label_name) in zip(info_labels_dict_iou.items(), decode_labels_dict.items()):
                        if len(iou_list) ==0:
                            continue
                        else:
                            iou_for_label_mean = np.round(sum(iou_list)/len(iou_list), decimals=4)
                        run[f"iou/{label_name}_{label}"].log(iou_for_label_mean, step=(val_num*10))
            ...
            if itr % opt.save_freq == 0 and itr>0:
